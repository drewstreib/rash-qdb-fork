...
function check_cmd_auth($params)
{
    ...
    /* 🛑 Read policy from $CONFIG, NEVER from $params.
       main() builds $params as array_merge($CONFIG, $_REQUEST, $_SESSION), so
       $_REQUEST overrides server config -- and these two values decide who may
       call what. Until 2026-08-14 `?cmd=queue&public_queue=1` therefore let an
       unauthenticated caller past the USER_MOD check on the moderation queue,
       verified against the live API. Request parameters must never set policy.
       ($addauth below is upstream's, assigned and never read; left alone.) */
    global $CONFIG;
